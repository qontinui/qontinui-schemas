...
class StateDescription(BaseModel):
    ...
    expected_elements: list[str] | None = Field(
        None,
        description=(
            "UI elements that should be visible in this state"
            " (e.g., 'Login button', 'Username field')"
        ),
    )
    unexpected_elements: list[str] | None = Field(
        None,
        description=(
            "UI elements that should NOT be visible in this state"
            " - helps detect error dialogs or wrong states"
        ),
    )
    user_goal: str | None = Field(
        None,
        description=(
            "Business context - what the user is trying"
            " to accomplish when in this state"
        ),
    )
...
class ActionDescription(BaseModel):
    ...
    intent: str = Field(
        ...,
        description=(
            "What this action is supposed to accomplish"
            " (e.g., 'Click the submit button to proceed')"
        ),
    )
    ...
    failure_modes: list[str] | None = Field(
        None,
        description=(
            "Known ways this action can fail"
            " (e.g., 'Button may be disabled', 'Network timeout')"
        ),
    )
...
class TransitionDescription(BaseModel):
    ...
    intent: str = Field(
        ...
        description=(
            ...
            " (e.g., 'Navigate from login to dashboard')"
        ),
    )
    preconditions: list[str] | None = Field(
        None,
    ...
    failure_modes: list[str] | None = Field(
        None,
        description=(
            "Known ways this transition can fail"
            " (e.g., 'Authentication error', 'Server unavailable')"
        ),
    )
...
class WorkflowDescription(BaseModel):
    ...
    success_criteria: str = Field(
        ...,
        description=(
            "How to determine if the workflow succeeded"
            " (e.g., 'User sees welcome message')"
        ),
    )
